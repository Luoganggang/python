...
class DeviceBalanceSVIP(QueryDeviceBalance):
    def __init__(self):
        super(QueryDeviceBalance,  self).__init__()
        self.set_log_level(self.LOGD)
        self.set_log_tag("DeviceBalanceSVIP")
    ...
    def execute(self):
        ...
        params = {"enableStatus":"1","bindStatus":"0","faultStatus":"0","grantOpenStatus":"on","maintStatus":"0", \
            "vmStatus":"1","padStatus":"1","rows":"1","padClassify":"4","padPool":"1"}
        ...
        #print ("new api svip data: %d" % lists['response']['totalNumber'])

        # 导入到数据库
        self.write_cursor.execute('INSERT INTO device_balance_svip (log_time, balance) VALUES (%s, %s)', \

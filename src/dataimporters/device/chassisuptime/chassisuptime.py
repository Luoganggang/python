...
class ChassisUpTime(DataImporter):
    def __init__(self):
        super(DataImporter,  self).__init__()
        self.set_log_level(self.LOGD)
        self.set_log_tag("ChassisUpTime")
    ...
    def execute(self):
        ...
        for ip_segment in dict_ip_segments:
            ...
            if ((len(rows) == 0) or (rows[0] == None)):
                ...
                idc = rows[0][0]
            sql = "INSERT INTO device_up_time_chassis "
            sql = sql + "(log_time, ip_segment, idc, up_time_too_short_total) VALUES "
            sql = sql + " (%s, %s, %s, %s)"
